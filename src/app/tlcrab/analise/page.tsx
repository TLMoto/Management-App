...
export default function TLCrabAnalise() {
  ...
  const getOverallStats = () => {
    ...
    const averagePeoplePerSlot = Object.values(availability).reduce((sum, slots) => sum + slots.length, 0) / filledSlots || 0;
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
              ...
              <div className="bg-blue-50 px-4 py-2 rounded-lg">
                ...
                {selectedPersonStats && (
                  <p className="text-xs text-blue-600">
                    {selectedPersonStats.totalHours}h ({selectedPersonStats.weekPercentage}% da semana)
                  </p>
                )}
              </div>
            )}
              ...
              <table className="min-w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {DAYS.map((day, index) => (
                      <th key={index} className="px-1 sm:px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[80px] sm:min-w-[100px]">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {TIME_SLOTS.map((timeSlot, slotIndex) => (
                    <tr key={slotIndex}>
                      ...
                      {DAYS.map((_, dayIndex) => {
                        const isAvailable = isPersonAvailable(dayIndex, timeSlot.hour, timeSlot.minute, selectedPerson);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-sm text-gray-600">{overallStats.filledSlots} de {overallStats.totalSlots} slots</p>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-600">
                        {stats.weekPercentage}% da semana
                      </p>
                    </div>
                  </div>
                  <div className="mt-2">
