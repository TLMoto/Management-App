...
export async function GET(request: NextRequest) {
    try {
        ...
        if (action === 'login' && eventId && personName) {
            ...
            const response = await fetch(`${API_BASE_URL}/event/${encodeURIComponent(eventId)}/people/${encodeURIComponent(personName)}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        return NextResponse.json(
            { error: 'Parâmetros inválidos. Use ?id=eventId para buscar evento ou ?eventId=X&personName=Y&action=login para login' },
            { status: 400 }
        );
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        if (!body.availability || !Array.isArray(body.availability)) {
            return NextResponse.json(
                { error: 'Disponibilidade inválida' },
                { status: 400 }
            );
        }
