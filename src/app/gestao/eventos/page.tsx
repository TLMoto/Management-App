...
export default function Eventos(): JSX.Element {
  ...
  const loadEventos = () => {
    EventosAtivosService.getEventosAtivos().then(eventos => {
      setEventos(eventos || []);
      console.log("Eventos Ativos:", eventos);
    });
  };

  const criarEvento = () => {
    EventosAtivosService.criarEvento({
      nome,
      dataInicio,
      dataFim,
    })
  ...
  const filteredEventos = eventos.filter(evento => {
    ...
    const matchesParticipante =
      filterParticipante === "" ||
      evento.participantes.some(p => p.toLowerCase().includes(filterParticipante.toLowerCase()));
    ...
    <ProtectedPage>
      <main className="min-h-screen flex flex-col pt-20 px-4 max-w-7xl mx-auto w-full">
        ...
        <div className="flex justify-between items-center mb-8">
          <button
            onClick={() => loadEventos()}
            className="bg-white hover:bg-gray-100 text-black px-6 py-2 rounded-lg font-medium transition"
          >
            ↻ Refrescar
          </button>
          <button
