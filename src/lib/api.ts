...
export async function updateAvailability(event_id: string, personName: string, availability: string[]): Promise<PersonAvailabilityResponse> {
    try {
        const response = await fetch(`${API_BASE_URL}?eventId=${encodeURIComponent(event_id)}&personName=${encodeURIComponent(personName)}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
...
export async function loginOrCreatePerson(event_id: string, personName: string): Promise<PersonAvailabilityResponse> {
    try {
        const response = await fetch(`${API_BASE_URL}?eventId=${encodeURIComponent(event_id)}&personName=${encodeURIComponent(personName)}&action=login`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
